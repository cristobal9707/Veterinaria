            ...
            <v-container fluid grid-list-lg>
                <v-layout align-space-around justify-center column class="form-contenedor">
                    <v-card style="padding: 10px;">
                        <h1>Receta: N°{{ $Recetas->id }}</h1>
                        <v-btn color="info" href="{{ url('/recetas') }}">Volver</v-btn>
                        <v-divider></v-divider>
                        <h3>Información de la Receta</h3>
                        <h5>Cliente: {{ $Recetas->clientes->nombre }}</h5>
                        <h5>Farmaco: {{ $Recetas->farmacos->nombre }}</h5>
                        <h5>Categoria: {{ $Recetas->farmacos->categoria }}</h5>
                        <h5>Cantidad: {{ $Recetas->cantidad }}</h5>
                        <h5>Precio: ${{ $Recetas->farmacos->precio }}</h5>
                        <h5>Comentario: {{ $Recetas->comentario }}</h5>
                        <h3>TOTAL: </h3>
                        <v-divider></v-divider>
                        <h3>Fechas</h3>
                        <h5>Fecha agregado: {{ $Recetas->created_at }}</h5>
                        <h5>Fecha editado: {{ $Recetas->updated_at }}</h5>
